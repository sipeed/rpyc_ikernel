...
.. image :: https://raw.githubusercontent.com/tdaff/rpyc_ikernel/master/doc/kernels.png
...
``rpyc_ikernel`` to manage kernels from different virtual environments or
different python implementations.

Install with ``pip install rpyc_ikernel``. Requires ``notebook`` (as part
...
.. warning::
   ...
   ``rpyc_ikernel`` opens multiple connections across several machines
...
.. code:: shell
   ...
   rpyc_ikernel manage --add --name="Python 3.8" --host=localhost --interface=rpyc

.. code:: shell

   # Add an SSH connection to a remote machine running IJulia

   rpyc_ikernel manage --add \
...
the drop-down list in the notebook. ``rpyc_ikernel manage`` also has options
...
================
  ...
  * Changed prefix to ``rpyc_``.
  * kernel_cmd now requires the ``{connection_file}`` argument.
  * ``rpyc_ikernel manage --show`` command to show existing kernels.
