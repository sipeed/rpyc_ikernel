...
class Client:
    ...
    def get_next_frame(self) -> Optional[Tuple[BytesIO, int]]:
    ...
    def _recv_rtp_packet(self, size=DEFAULT_CHUNK_SIZE) -> RTPPacket:
        recv = bytes()
        logging.debug('Waiting RTP packet...')
        while True:
            try:
    ...
    def establish_rtsp_connection(self):
        if self.is_rtsp_connected:
            logging.debug('RTSP is already connected.')
            return
        logging.debug(f"Connecting to {self.remote_host_address}:{self.remote_host_port}...")
    ...
    def close_rtsp_connection(self):
        if not self.is_rtsp_connected:
            logging.debug('RTSP is not connected.')
            return
        if self._rtp_socket:
    ...
    def _send_request(self, request_type=RTSPPacket.INVALID) -> RTSPPacket:
        ...
        logging.debug(f"Sending request: {repr(request)}")
    ...
    def _get_response(self, size=DEFAULT_CHUNK_SIZE) -> RTSPPacket:
        ...
        while True:
            try:
                ...
                continue
        # logging.debug(f"Received from server: {repr(rcv)}")
